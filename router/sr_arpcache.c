...
void sr_arpcache_sweepreqs(struct sr_instance *sr) {
  for (struct sr_arpreq *req = sr->cache.requests; req != NULL; req = req->next) handle_arpreq(sr, req);
}
...
uint8_t *create_arp_request(struct sr_instance *sr, uint32_t ip, const char *iface) {
  ...
  arp_hdr->ar_pro = htons(ethertype_ip);      // IP
  arp_hdr->ar_hln = ETHER_ADDR_LEN;           // MAC address length
  arp_hdr->ar_pln = sizeof(uint32_t);         // IP address length
